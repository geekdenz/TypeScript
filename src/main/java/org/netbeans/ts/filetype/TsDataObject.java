package org.netbeans.ts.filetype;
...
@MIMEResolver.ExtensionRegistration(
        displayName = "#LBL_Ts_LOADER",
        mimeType = "text/x-ts",
        extension = {"ts"}
)
@DataObject.Registration(
        mimeType = "text/x-ts",
        iconBase = "org/netbeans/ts/filetype/ts.png",
        displayName = "#LBL_Ts_LOADER",
        position = 300
)
@ActionReferences({
    @ActionReference(
            path = "Loaders/text/x-ts/Actions",
...
public class TsDataObject extends MultiDataObject {
    ...
    public TsDataObject(FileObject pf, MultiFileLoader loader) throws DataObjectExistsException, IOException {
        super(pf, loader);
        registerEditor("text/x-ts", false);
    }
